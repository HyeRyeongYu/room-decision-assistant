...
| 2026-05-04 | **0.5** | User Interface Prototype |
| 2026-05-05 | **0.6** | Glossary |
| 2026-05-05 | **0.7** | References |
<br>
...
- 따라서, 본 시스템에서는 주거 매물 선택 과정에서 발생할 수 있는 문제와 어려움을 해결하기 위해, 법적 문서를 기반으로 하는 체크리스트를 제공한다. 이를 통해, 사용자의 법률 용어에 대한 장벽을 낮추고, 다양한 후보 매물을 체계적으로 평가하고, 비교할 수 있도록 한다. 또한, 의사결정 과정의 정확도를 높여, 중개 계약 이후의 리스크를 줄이는 것을 목표로 하며, 향후 다양한 주거 유형과 사용자 요구를 반영하여 확장이 가능하도록 설계한다.
...
- 이때, Actor와 상호작용을 하는 Use case는 Association 관계로 연결했으며, 기본 Use case 수행 중에 해당 Use case가 선택적으로 실행될 수 있는 경우 Extend 관계로 연결했다.
...
| Failed Post Condition | 주거 비용 정보가 입력되지 않았거나 일부만 입력된 경우, 해당 항목은 저장되지 않으며 이전 상태가 유지된다. |
...
| Primary Actor | User |
| Preconditions | 체크리스트가 생성되어 있어야 하며, 사용자가 해당 체크리스트를 선택한 상태여야 한다. |
| Trigger | 사용자가 체크리스트 항목에 대해 상/중/하 평가 중 하나를 선택하거나, 총점수 계산 버튼을 클릭한다. |
| Success Post Condition | 총점수와 상위 평가 항목 전체 개수가 계산되어 체크리스트에 반영되며,
사용자는 계산된 평가 결과를 확인할 수 있다. |
...
| 6 | 시스템은 사용자에게 계산된 총점수와 상위 평가 항목 전체 개수를 팝업 메시지로 보여준다. |
...
|------|------|
| Performance | ≦ 1 Seconds |
| Frequency | 사용자가 체크리스트에서 항목을 평가한 후 총점수 계산 버튼을 클릭할 때마다 |
| Concurrency | Not applicable (single-user browser environment) |
| Due Date | 2026-05-03 |
<br> 
...
| 항목 | 내용 |
|------|------|
| Summary | 사용자가 기본 체크리스트 외에 추가로 맞춤 항목 입력을 원하는 경우, 사용자 맞춤 항목을 추가하여 체크리스트에 반영한다. |
| Scope | Room Checkmate System |
| Level | User level |
| Author | HR Yu |
| Last Update | 2026-05-03 |
...
| 3b | **사용자가 기존 사용자 맞춤 항목을 삭제하려는 경우** |
| 3b1 | 사용자가 삭제할 사용자 맞춤 항목을 선택한다. |
| 3b2 | 시스템은 해당 맞춤 항목을 삭제한다. |
| 3b3 | 시스템은 사용자에게 사용자 맞춤 항목 삭제 완료 팝업 메시지를 보여준다. |
...
|------|------|
| Performance | ≦ 1 Seconds |
| Frequency | 사용자가 체크리스트 화면에서 맞춤 항목 추가 버튼을 누를 때마다 |
| Concurrency | Not applicable (single-user browser environment) |
| Due Date | 2026-05-03 |
<br>
...
| 항목 | 내용 |
|------|------|
| Summary | 사용자가 후보 매물에 대한 관찰 사항, 매물 확인 후의 생각, 특이 사항 등을 메모로 작성하여 체크리스트에 반영한다. |
| Scope | Room Checkmate System |
| Level | User level |
| Author | HR Yu |
| Last Update | 2026-05-03 |
...
| Success Post Condition | 선택된 체크리스트에 대한 비교 결과가 사용자가 선택한 비교 기준에 따라 생성되어 사용자에게 제공된다. |
...
| Trigger | 사용자가 추천 기능 화면에서 최적 매물 추천 버튼을 클릭한다. |
...
| 5 | 시스템은 각 체크리스트의 총점수를 기반으로 우선 비교한다. |
...
| 항목 | 내용 |
|------|------|
| Summary | 사용자가 현재까지 입력 및 평가한 체크리스트의 모든 내용을 저장하여, 시스템에 반영한다. |
| Scope | Room Checkmate System |
| Level | User level |
| Author | HR Yu |
| Last Update | 2026-05-03 |
...
## Use Case #9 : Save checklist data (체크리스트 외부 파일로 저장)
### [GENERAL CHARACTERISTICS]

| 항목 | 내용 |
|------|------|
| Summary | 사용자가 선택한 체크리스트의 모든 내용이 외부 파일 형태로 저장하여 다운로드한다. |
| Scope | Room Checkmate System |
| Level | User level |
| Author | HR Yu |
| Last Update | 2026-05-03 |
...
## Use Case #10 : Load checklist data (체크리스트 외부 파일 불러오기) 
### [GENERAL CHARACTERISTICS]

| 항목 | 내용 |
|------|------|
| Summary | 사용자가 외부 파일 형태로 저장된 체크리스트 데이터를 불러와 기존의 체크리스트 목록에 추가한다. |
| Scope | Room Checkmate System |
| Level | User level |
| Author | HR Yu |
| Last Update | 2026-05-03 |
...
- 해당 클래스는 총점수 평가 결과와 상위 평가 항목 전체 개수 결과를 속성으로 가진다.
...
- 해당 클래스는 총점수 기반 비교와 상위 평가 항목 개수 기반 비교 기능을 수행한다.
- Room Checkmate 시스템은 해당 클래스를 통해 비교 기준 유형에 따른 체크리스트 간 비교 결과를 생성한다.

## 9) CompareResult
- CompareResult 클래스는 여러 체크리스트를 비교한 결과를 나타내는 클래스이다.
- 해당 클래스는 총점수 기반 비교 결과와 상위 평가 항목 개수 기반 비교 결과를 속성으로 가진다.
- Room Checkmate 시스템은 해당 클래스를 기반으로 사용자가 개별 매물 간 상대적 차이를 확인할 수 있도록 정보를 제공한다.

## 10) RecommendationEngine
- RecommendationEngine 클래스는 최적 매물 추천 기능을 수행하는 클래스이다.
- 해당 클래스는 총점수, 사용자 맞춤 항목, 주거 비용 등의 평가 결과를 기반으로 추천 결과를 생성한다.
...
- Room Checkmate 시스템은 해당 클래스를 비교 결과를 기반으로 사용자에게 가장 적합한 매물을 제시한다.
  
## 12) FileData
- FileData 클래스는 체크리스트의 데이터를 저장 및 불러오기 위한 클래스이다.
- 해당 클래스는 로컬 스토리지 저장과 외부 파일로 저장 및 불러오기를 위한 데이터 구조를 포함한다.
...
[Figure 2. Home Interface]<br><br>

- 사용자가 Room Checkmate 웹사이트에 접속하면, 홈 화면이 표시된다.
- 해당 화면에서는 체크리스트, 비교&추천, 내 체크리스트 관리 메뉴를 통해 각각의 기능으로 이동할 수 있으며, 사용자는 홈 화면을 통해 전체 기능에 접근할 수 있다.

## 4.2 Checklist Creation Interface
<img src="./images/ui_create.png" width="700">
[Figure 3. Checklist Creation Interface]<br><br>

- 사용자가 홈 화면에서 체크리스트 메뉴를 선택하면 해당 화면으로 이동한다.
- 이 화면에서는 평가하고자 하는 매물의 체크리스트를 생성할 수 있다.
- 사용자는 매물 유형과 계약 유형을 선택하고, 체크리스트의 이름을 작성한 뒤, 체크리스트 생성 버튼을 클릭하면, 선택한 유형에 따라 기본 체크리스트가 자동으로 생성된다.
- 이때, 매물 유형, 계약 유형, 체크리스트 이름의 모든 기본 항목이 입력되어야 생성할 수 있으므로, 입력이 누락되면 시스템은 팝업 메시지를 통해 작성을 요청한다.

## 4.3 Evaluation Interface
- 사용자가 체크리스트를 생성하거나 선택하면 평가 화면이 표시된다.
- 해당 화면에서는 사용자가 체크리스트의 각 항목에 대해 상/중/하 기준으로 평가할 수 있다.
- 또한, 상단의 총점수 계산 버튼을 통해 총점수와 상위 평가 항목의 전체 개수를 확인할 수 있다.

---

### 4.3.1 Housing Cost Interface
<img src="./images/ui_evaluation1.png" width="700">
[Figure 4. Housing Cost Interface]<br><br>

- 사용자가 체크리스트를 생성하면 기본 체크리스트 화면이 표시된다.
- 해당 화면의 주거 비용 그룹에서는 매물 거래 시 필요한 비용 정보를 작성할 수 있다. 
- 또한, 화면에는 법률 관련 설명과 용어 부가 설명이 함께 제공되어, 사용자가 항목을 이해하고, 평가할 수 있도록 지원한다.

### 4.3.2 Internal State Interface
<img src="./images/ui_evaluation2.png" width="700">
[Figure 5. Internal State Interface]<br><br>

- 해당 화면의 내부 시설물 상태 그룹에서는 매물의 내부 상태와 관련된 항목들을 평가할 수 있다.
- 또한, 사용자는 각 항목은 상태 정도 및 세부 조건에 따라 선택하여 매물의 내부 상태를 평가할 수 있다.

### 4.3.3 External State Interface
<img src="./images/ui_evaluation3.png" width="700">
[Figure 6. External State Interface]<br><br>

- 해당 화면의 외부 시설물 상태 그룹에서는 매물의 외부 상태와 관련된 항목들을 평가할 수 있다.
- 사용자는 승강기, 관리실, CCTV, 비상구 등의 외부 시설 존재 여부 및 상태를 기준으로 평가를 수행할 수 있다.

### 4.3.4 Environment State Interface
<img src="./images/ui_evaluation4.png" width="700">
[Figure 7. Environment State Interface]<br><br>

- 해당 화면의 환경 상태 그룹에서는 매물의 내·외부 환경 요소와 관련된 항목들을 평가할 수 있다.
- 사용자는 각 항목을 상태 정도에 따라 평가할 수 있으며, 각 항목에 대한 평가 이유도 함께 작성할 수 있는 입력란이 함께 제공된다.


### 4.3.5 Location Conditions Interface
<img src="./images/ui_evaluation5.png" width="700">
[Figure 8. Location Conditions Interface]<br><br>

- 해당 화면의 입지 조건 그룹에서는 매물에 직간접적인 영향을 미치는 위치적 요소와 관련된 항목을 평가할 수 있다. 
- 사용자는 대중교통 접근성, 주차 환경, 주변 시설 등과 같은 입지 조건 항목을 선택하여, 매물의 위치적 특성을 평가할 수 있다.  

### 4.3.6 Customize Interface
<img src="./images/ui_evaluation6.png" width="700">
[Figure 9. Customize Interface]<br><br>

- 해당 화면의 사용자 맞춤 그룹에서는 사용자가 매물과 관련된 추가 항목을 생성하고 평가할 수 있다.
- 사용자는 화면의 상단에 있는 맞춤 항목 추가 버튼을 통해 새로운 평가 항목을 생성할 수 있으며, 이때 추가 가능한 최대 개수 내에서 항목이 생성된다.
- 생성된 항목에 대해 사용자는 항목 이름을 입력하고 평가를 수행할 수 있으며, 각 항목에 대한 관련 정보를 입력할 수 있는 비고란이 함께 제공된다. 

### 4.3.7 Memo Interface
<img src="./images/ui_evaluation7.png" width="700">
[Figure 10. Memo Interface]<br><br>

- 해당 화면의 메모 그룹에서는 사용자가 매물과 관련된 생각이나 의견 등을 자유롭게 작성할 수 있다.
- 또한, 사용자는 필요에 따라 항목을 구분하여 내용을 입력할 수 있다.

## 4.4 Comparison&Recommendation Interface
<img src="./images/ui_compareNrecommend.png" width="700">
[Figure 11. Comparison&Recommendation Interface]<br><br>

- 사용자가 홈 화면에서 비교&추천 메뉴를 선택하면 해당 화면으로 이동한다.
- 해당 화면에서는 시스템에 저장된 사용자의 체크리스트 목록이 제공되며, 사용자는 최대 허용 개수 내에서 비교 또는 추천 대상 체크리스트를 선택할 수 있다.
- 또한, 사용자는 비교 또는 추천 버튼을 선택할 수 있으며, 선택한 기능에 따라 결과를 확인할 수 있다.

## 4.5 Compare Interface
### 4.5.1 Compare Interface (Based on Total Score)
<img src="./images/ui_compare1.png" width="700">
[Figure 12. Compare Interface (Based on Total Score)]<br><br>

- 사용자가 비교&추천 메뉴에서 총점수 기반 비교 버튼을 클릭하면 해당 화면으로 이동한다.
- 이 화면에서는 선택한 체크리스트의 총점수와 주거 비용을 함께 제시하여, 이를 기반으로 사용자가 후보 매물을 정량적으로 비교할 수 있다.

### 4.5.2 Compare Interface (Based on Top Evaluation Count)
<img src="./images/ui_compare2.png" width="700">
[Figure 13. Compare Interface (Based on Top Evaluation Count)]<br><br>

- 사용자가 비교&추천 메뉴에서 상위 평가 항목 개수 비교 버튼을 클릭하면 해당 화면으로 이동한다.
- 이 화면에서는 각 그룹의 총평가 항목 개수 대비 사용자가 선택한 상위 평가 항목 개수를 제시하여, 이를 기반으로 사용자는 후보 매물 간 상대적인 평가 수준을 비교할 수 있다.

## 4.6 Recommendation Interface
<img src="./images/ui_recommend.png" width="700">
[Figure 14. Recommendation Interface]<br><br>

- 사용자가 비교&추천 메뉴에서 최적 매물 추천 버튼을 클릭하면 해당 화면으로 이동한다.
- 이 화면에서는 선택한 체크리스트를 기반으로 가장 적합한 매물을 추천한다.
- 추천 결과 화면에는 총점수, 주거 비용, 사용자 맞춤 항목 반영 결과 등이 함께 표시된다.
- 또한, 해당 매물이 선택된 이유를 함께 제공하여 사용자가 추천 결과를 이해할 수 있도록 지원한다.

## 4.7 Management Interface
<img src="./images/ui_manage.png" width="700">
[Figure 15. Management Interface]<br><br>

- 사용자가 홈 화면에서 내 체크리스트 관리 메뉴를 선택하면 해당 화면으로 이동한다.
- 이 화면에서는 시스템에 저장된 사용자의 체크리스트 전체 목록을 확인할 수 있다.
- 또한, 외부 파일로 저장하거나 기존 파일을 불러오는 기능을 제공한다.
- 사용자는 외부 파일로 저장 기능을 통해 작성한 체크리스트를 파일 형태로 다운로드하여 보관할 수 있으며, 외부 파일 불러오기 기능을 통해 이전에 저장한 체크리스트를 다시 시스템에 추가할 수 있다.
<br>
...
| 확인·설명서 | 개업 공인중개사가 중개대상물(매물)의 상태, 권리관계, 법적 제한사항 등을 중개 완성 전 확인하여, 거래 당사자(임차인)에게 서면으로 교부하는 법적 문서 |
...
# 6. References
- 중개대상물 확인설명서 (주거용 건축물) (2020.2.21.)<br>
https://pheo.co.kr/?mid=pdsform&page=5&document_srl=136166

- 중개대상물 확인설명서 작성 예시<br>
https://blog.naver.com/cbjk9326/221389095140

- 중개대상물 확인설명서 개정 사항 (2024.07.10.)<br>
https://m.blog.naver.com/yduuyn/223504637536
https://www.law.go.kr/LSW//lumLsLinkPop.do?ancYnChk=0&chrClsCd=010202&lspttninfSeq=106739
