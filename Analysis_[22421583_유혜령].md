...
- 해당 화면에서는 사용자가 체크리스트의 각 항목을 평가할 수 있다.
...
- 해당 화면의 주거 비용 그룹에서는 매물 거래 시 필요한 비용을 작성할 수 있다.
- 또한, 법률 관련 설명과 용어 부가 설명을 참고하여 사용자는 체크리스트 항목의 평가를 수행할 수 있다.
...
- 또한, 각 항목은 상태 정도 및 세부 조건에 따라 선택하여 평가할 수 있다.
...
- 사용자는 항목을 상태 정도에 따라 평가할 수 있으며, 평가에 대한 이유도 함께 작성할 수 있다.
...
- 해당 화면의 입지 조건 그룹에서는 매물에 직간접적인 영향을 미치는 위치적 요소를 평가할 수 있다.
- 따라서, 사용자는 대중교통 접근성, 주차 환경, 주변 시설 등의 입지 조건을 기준으로 평가를 수행할 수 있다.
...
- 사용자는 화면의 상단에 있는 맞춤 항목 추가 버튼을 통해 새로운 평가 항목을 추가할 수 있으며, 추가 가능한 최대 개수 내에서 항목이 생성된다.
- 생성된 항목에 대해 사용자는 항목 이름을 입력하고 평가를 수행할 수 있으며, 관련 정보는 비고란에 함께 작성할 수 있다.
...
- 또한, 사용자는 필요에 따라 항목을 구분하여 메모를 작성할 수 있다.
...
- 이 화면에서는 시스템에 저장된 사용자의 체크리스트를 최대 개수 내로 선택할 수 있다.
...
- 이 화면에서는 각 그룹의 총평가 항목 개수 대비 사용자가 선택한 상위 평가 항목 개수를 제시하여, 이를 기반으로 사용자는 후보 매물을 비교할 수 있다.
...
- 추천 결과에는 총점수, 주거 비용, 사용자 맞춤 항목 반영 결과 등이 함께 제시된다.
- 또한, 해당 매물이 선택된 이유를 함께 제공하여 사용자가 결과를 이해할 수 있도록 한다.
...
- 외부 파일로 저장 기능을 통해 작성한 체크리스트를 파일 형태로 보관할 수 있으며, 외부 파일 불러오기 기능을 통해 이전에 저장한 체크리스트를 다시 시스템에 불러올 수 있다.
<br>
